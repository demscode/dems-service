...
describe('Level API', function() {
  var model = require('../../models/level').init(settings.db.mongo);
  var level0 = {
    levelValue: 0,
    name: "Low"
  };
  ...
  it('GET a valid Level', function(done) {
    request.get('/api/level/0/').expect(200, level0);
    done();
  });

  it('GET an invalid Level', function(done) {
    request.get('/api/level/3/').expect(404);
    done();
  });

  it('DELETE a valid Level', function(done) {
    request.delete('/api/level/0/').expect(200);
    done();
  });

  it('DELETE an invalid Level', function(done) {
    request.delete('api/level/3/').expect(404);
    done();
  });
});
});
